...
# ↓ 适应度函数
def fitness(x):
    return np.sin(x[0]) * np.cos(x[1]) - np.sqrt(x[0]**2 + x[1]**2)/2
...
class individual:
    def __init__(self):
        # ↓ 染色体编码
        self.x = np.array([np.random.uniform(-10, 10), np.random.uniform(-10, 10)])
        # ↓ 适应度值
...
if __name__ == '__main__':
    ...
    print(len(pop))
    for each in pop:
        print('x1=%f,x2=%f\ty=%f' % (each.x[0], each.x[1], each.fitness))
